...
일단 현재는 NCP 사용에 만족하고 있고, 계속 사용하면서 고도화를 진행 중입니다. UI/UX에서 약간의 아쉬움은 있지만, 문서나 기능은 꽤 충실하다는 느낌이 들어 몇 개월간은 무리 없이 사용 가능할 것으로 보입니다. 내부적으로는 이후 AWS로 마이그레이션 여부도 함께 검토할 예정입니다.
